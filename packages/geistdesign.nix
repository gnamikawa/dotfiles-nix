...
let
  ...
  cssValue =
    path: value: if lib.last path == "duration" then "${toString value}ms" else toString value;

  properties =
    path: values:
    if lib.isAttrs values then
      values |> lib.attrNames |> lib.concatMapStrings (name: properties (path ++ [ name ]) values.${name})
    else
  ...
  typeClasses =
    let
      perFamily =
        family:
        constants.type.${family}
        |> lib.attrNames
        |> lib.concatMapStrings (
          size: ".text-${family}-${size} {\n${typeProperties constants.type.${family}.${size}}}\n\n"
        );
    in
    [
      "heading"
    ...
    |> lib.concatMapStrings perFamily;
